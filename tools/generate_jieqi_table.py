...
OUTPUT_PATH = os.getenv(
    "JIEQI_OUTPUT",
    os.path.join("data", "jieqi_1900_2052.json")
)
...
def sun_lon_deg(eph, ts, t):
    ...
    lon, _, _ = sun.frame_latlon(ecliptic_frame)
    return lon.degrees % 360.0


def find_crossing(eph, ts, target_deg, t0, t1):
    """
    Binary search for the time when solar ecliptic longitude
    crosses target_deg between t0 and t1.
    """
    def f(t):
    ...
    lo, hi = t0, t1
    for _ in range(60):
        mid_dt = lo.utc_datetime() + (hi.utc_datetime() - lo.utc_datetime()) / 2
        mid = ts.from_datetime(mid_dt)
        if f(lo) * f(mid) <= 0:
            hi = mid
        else:
            lo = mid
...
def generate_year(eph, ts, year: int):
    ...
    # sample one year daily to bracket crossings
...
def generate():
    ...
    for year in range(START_YEAR, END_YEAR + 1):
        ...
        print(
            f"[DEBUG] generate_year({year}) returned {len(year_data)} items",
            flush=True
        )
